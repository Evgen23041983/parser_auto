...
final class Database { //final - значит нельзя этот класс наследовать.
	...
	public function __construct($host,$user,$pass,$db) {
		...
		if(!mysqli_select_db($this->db,$db)) exit('Нет таблицы');
	...
	public function get_data($id, $data, $table) {
		...
		$id=$this->sanitize($id);
		 ...
		 $ResultQuery=mysqli_query ("SELECT 
                                            *
                                        FROM
                                            $table 				  	
                                       WHERE 
	...
	*/
	public function insert($array_params, $table) {
		...
		return mysqli_query($this->db, $sql)  or die("<br>Ошибка при добавлении в таблицу <strong>$table</strong>:<br>". mysql_error()."<br>");
	}
	...
	*/
	public function update($array_params, $array_filter, $table) {
		...
		return mysqli_query($sql)  or die("<br>Ошибка при изменении в таблицы <strong>$table</strong>:<br>". mysql_error()."<br>");
	}
	...
	*/
	public function last() {
		...
		$r = mysqli_query($sql);
		while ($Result=mysqli_fetch_array($r)) return $Result[0]; 
	}
